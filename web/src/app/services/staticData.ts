...
const STATIC_BASE_CANDIDATES: StaticBase[] = ["", "/public", "/web", "/web/public"];
...
const inferStaticBaseFromResolvedPath = (resolvedPath: string, originalPath: string): StaticBase => {
  ...
  const base = resolvedPath.slice(0, resolvedPath.length - normalizedPath.length);
  if (base === "" || base === "/public" || base === "/web" || base === "/web/public") {
    return base;
  }
  ...
  const promise = (async () => {
    ...
    for (const resolvedPath of buildCandidatePaths(path)) {
      try {
        ...
        preferredStaticBase = inferStaticBaseFromResolvedPath(resolvedPath, path);
        return parsed;
      } catch (error) {
...
export const loadMetadata = async (): Promise<MetadataPayload> => {
  if (!metadataPromise) {
    metadataPromise = fetchJson<MetadataPayload>("/data/metadata.json").catch((error) => {
...
const findBook = (
  ...
  return books.find((book) => {
    ...
    return candidates.some((candidate) => normalizeBookToken(candidate) === target);
  });
};
...
const loadDssChapter = async (
  ...
  try {
    ...
    return Object.entries(chapterData.verses).reduce((acc, [verseKey, verseValue]) => {
