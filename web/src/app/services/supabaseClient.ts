...
const warnNetworkFailureOnce = (error: unknown) => {
  ...
  console.warn("TS2009 network unavailable; using fallback translation sources.", error);
};
...
const extractChapterVerseEntries = (
  ...
  if (Array.isArray(chaptersRaw)) {
    ...
    if (isRecord(chapterObjectMatch) && Array.isArray(chapterObjectMatch.verses)) {
...
const loadChapterFromBookFile = async (
  ...
  if (chapterBookCache.has(cacheKey)) {
    return chapterBookCache.get(cacheKey) as Promise<Map<number, string> | null>;
  }
  ...
  const loader = (async () => {
    ...
    for (const candidate of candidates) {
      ...
      try {
        ...
        const chapterVerses = extractChapterVerseEntries(parsed.chapters, chapter);
