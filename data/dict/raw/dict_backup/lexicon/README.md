# Hebrew Scripture Lexicon - Data Repository

This directory contains the Hebrew Scripture lexicon data with complete word definitions,
morphological analysis, and biblical references.
...
│
├── words/                       # Lexicon entries (derived words)
│   ├── H1.json                  # אָב (father)
│   ├── H428.json                # אֵלֶּה (these)
│   └── ... (6315+ entries)
│
...
│   └── ... (3133+ entries)
│
└── testing/                     # Testing environment (1% sample)
    ├── words/
    └── roots/
```
...
### Main Script: `scripts/dict/build_lexicon.py`

The main lexicon building script is now located in `scripts/dict/build_lexicon.py` and builds complete lexicon entries with:
...
python scripts/dict/build_lexicon.py H7965 --update
```

#### 2. Batch Processing
```bash
# Process all entries from JSON file
python scripts/dict/build_lexicon.py lexicon/lexicon_100_percent_list.json

# Update all existing entries
python scripts/dict/build_lexicon.py lexicon/lexicon_100_percent_list.json --update
...
python scripts/dict/build_lexicon.py lexicon/lexicon_100_percent_list.json --testing

# Testing with update
python scripts/dict/build_lexicon.py lexicon/lexicon_100_percent_list.json --testing --update
```

**Testing mode features:**
- ✅ Automatically limits to 1% of data
- ✅ Saves to `lexicon/testing/words/` and `lexicon/testing/roots/`
- ✅ Doesn't modify existing data in `lexicon/words/` and `lexicon/roots/`
...
# Only process words/ directory
python scripts/dict/build_lexicon.py --fill-missing --draft-only

# Only process testing/ directory
python scripts/dict/build_lexicon.py --fill-missing --testing-only
```
...
- ✅ Processes `words/` and `testing/` directories by default
