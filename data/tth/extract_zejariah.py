#!/usr/bin/env python3
"""
Extract Zejariah (Zechariah) from tanaj.docx and create zejariah.md
...
def extract_zejariah_section(text: str) -> str:
    """Extract the Zejariah section from the full text."""
    lines = text.split('\n')
    ...
    for i, line in enumerate(lines):
        if line.strip() == '__ZEJARIÁH \\(ZACARÍAS\\)__ זכריה* *':
            zejariah_start = i
            print(f"Found Zejariah start at line {i}")
            break

    if zejariah_start == -1:
        print("Error: Zejariah title not found")
        return ""

    # Find Zejariah end (next book: Malaji/Malachi)
    zejariah_end = -1
    for i in range(zejariah_start + 1, len(lines)):
        line = lines[i]
        ...
        # Stop at the main title of the next book (Malaji/Malachi)
        if 'MALAJI' in line_upper or 'MALAJÍ' in line_upper or 'MALACHI' in line_upper or 'מלאכי' in line:
            zejariah_end = i
            print(f"Found Zejariah end at line {i}: {line[:50]}...")
            break
    ...
    if zejariah_end == -1:
        ...
        zejariah_end = len(lines)

    # Extract Zejariah lines
    zejariah_lines = lines[zejariah_start:zejariah_end]

    # Find all footnote numbers used in Zejariah
    footnote_nums = set()
    zejariah_text = '\n'.join(zejariah_lines)
    # Find footnote references like [^1]
    for match in re.finditer(r'\[\^(\d+)\]', zejariah_text):
        footnote_nums.add(int(match.group(1)))

    print(f"Found {len(footnote_nums)} unique footnotes in Zejariah")

    # Extract footnote definitions that belong to Zejariah
    footnote_lines = []
    if footnote_nums:
        for i in range(zejariah_end, len(lines)):
            line = lines[i]
    ...
    return '\n'.join(zejariah_lines + footnote_lines)
...
def format_to_target_structure(text: str) -> str:
    ...
    while i < len(lines):
        ...
        if (line_length < 150 and line_length > 0 and '__' in line and
            ...
            # Check for individual book titles (like Zejariah)
            if line.strip() == '__ZEJARIÁH \\(ZACARÍAS\\)__ זכריה* *':
                book_title_found = '__ZEJARIÁH (ZACARÍAS)*__ זכריה*'
...
def main():
    ...
    output_file = "zejariah.md"

    if not os.path.exists(input_file):
        print(f"Error: Input file '{input_file}' does not exist.")
        sys.exit(1)

    print(f"Converting '{input_file}' to extract Zejariah section...")
    ...
    print("Extracting Zejariah section...")
    zejariah_text = extract_zejariah_section(normalized_text)

    if not zejariah_text:
        print("Error: Could not extract Zejariah section")
        sys.exit(1)

    print("Applying minimal formatting...")
    formatted_text = format_to_target_structure(zejariah_text)
    ...
    print("✓ Zejariah extraction completed successfully!")
