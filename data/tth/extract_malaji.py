#!/usr/bin/env python3
"""
Extract Malaji (Malachi) from tanaj.docx and create malaji.md
...
def extract_malaji_section(text: str) -> str:
    """Extract the Malaji section from the full text."""
    lines = text.split('\n')
    ...
    for i, line in enumerate(lines):
        if line.strip() == '__MALAJÍ \\(MALAQUÍAS\\)__מלאכי* *':
            malaji_start = i
            print(f"Found Malaji start at line {i}")
            break

    if malaji_start == -1:
        print("Error: Malaji title not found")
        return ""

    # Find Malaji end (transition to Ketuvim/Salmos)
    malaji_end = -1
    for i in range(malaji_start + 1, len(lines)):
        line = lines[i]
        ...
        # Stop at the transition to Ketuvim/Salmos
        if 'KETUVIM' in line_upper or 'SALMOS' in line_upper or 'TEHILIM' in line_upper or 'תהלים' in line:
            malaji_end = i
            print(f"Found Malaji end at line {i}: {line[:50]}...")
            break

    # If no transition found, take everything until footnotes start
    if malaji_end == -1:
        for i in range(malaji_start + 1, len(lines)):
    ...
    if malaji_end == -1:
        ...
        malaji_end = len(lines)

    # Extract Malaji lines
    malaji_lines = lines[malaji_start:malaji_end]

    # Find all footnote numbers used in Malaji
    footnote_nums = set()
    malaji_text = '\n'.join(malaji_lines)
    # Find footnote references like [^1]
    for match in re.finditer(r'\[\^(\d+)\]', malaji_text):
        footnote_nums.add(int(match.group(1)))

    print(f"Found {len(footnote_nums)} unique footnotes in Malaji")

    # Extract footnote definitions that belong to Malaji
    footnote_lines = []
    if footnote_nums:
        for i in range(malaji_end, len(lines)):
            line = lines[i]
    ...
    return '\n'.join(malaji_lines + footnote_lines)
...
def format_to_target_structure(text: str) -> str:
    ...
    while i < len(lines):
        ...
        if (line_length < 150 and line_length > 0 and '__' in line and
            ...
            # Check for individual book titles (like Malaji)
            if line.strip() == '__MALAJÍ \\(MALAQUÍAS\\)__מלאכי* *':
                book_title_found = '__MALAJÍ (MALAQUÍAS)*__מלאכי*'
...
def main():
    ...
    output_file = "malaji.md"

    if not os.path.exists(input_file):
        print(f"Error: Input file '{input_file}' does not exist.")
        sys.exit(1)

    print(f"Converting '{input_file}' to extract Malaji section...")
    ...
    print("Extracting Malaji section...")
    malaji_text = extract_malaji_section(normalized_text)

    if not malaji_text:
        print("Error: Could not extract Malaji section")
        sys.exit(1)

    print("Applying minimal formatting...")
    formatted_text = format_to_target_structure(malaji_text)
    ...
    print("✓ Malaji extraction completed successfully!")
