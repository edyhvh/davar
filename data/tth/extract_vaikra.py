#!/usr/bin/env python3
"""
Extract Vaikrá (Leviticus) from tanaj.docx and create vaikra.md
Based on the optimized Bereshit and Shemot extraction approaches
"""
...
def extract_vaikra_section(text: str) -> str:
    """Extract the Vaikrá section from the full text."""
    lines = text.split('\n')
    ...
    for i, line in enumerate(lines):
        if line.strip() == '__VAIKRÁ \\(LEVÍTICO\\)__ ויקרא':
            vaikra_start = i
            print(f"Found Vaikrá start at line {i}")
            break

    if vaikra_start == -1:
        print("Error: Vaikrá title not found")
        return ""

    # Find Vaikrá end (next book: BAMIDBAR/Números)
    vaikra_end = -1
    for i in range(vaikra_start + 1, len(lines)):
        line = lines[i]
        ...
        # Stop at the main title of the next Torah book (Bamidbar/Numbers)
        if line.strip() == '__BAMIDBAR \\(NÚMEROS\\)__במדבר':
            vaikra_end = i
            print(f"Found Vaikrá end at line {i}")
            break
    ...
    if vaikra_end == -1:
        ...
        vaikra_end = len(lines)

    # Extract Vaikrá lines
    vaikra_lines = lines[vaikra_start:vaikra_end]

    # Find all footnote numbers used in Vaikrá
    footnote_nums = set()
    vaikra_text = '\n'.join(vaikra_lines)
    # Find footnote references like [^1]
    for match in re.finditer(r'\[\^(\d+)\]', vaikra_text):
        footnote_nums.add(int(match.group(1)))

    print(f"Found {len(footnote_nums)} unique footnotes in Vaikrá")

    # Extract footnote definitions that belong to Vaikrá
    footnote_lines = []
    if footnote_nums:
        for i in range(vaikra_end, len(lines)):
            line = lines[i]
    ...
    return '\n'.join(vaikra_lines + footnote_lines)
...
def format_to_target_structure(text: str) -> str:
    ...
    while i < len(lines):
        ...
        if (line_length < 150 and line_length > 0 and '__' in line and
            ...
            # Check for individual book titles (like Vaikrá)
            elif line.strip() == '__VAIKRÁ \\(LEVÍTICO\\)__ ויקרא':
                book_title_found = '__VAIKRÁ (LEVÍTICO)*__ויקרא*'
...
def main():
    ...
    output_file = "vaikra.md"

    if not os.path.exists(input_file):
        print(f"Error: Input file '{input_file}' does not exist.")
        sys.exit(1)

    print(f"Converting '{input_file}' to extract Vaikrá section...")
    ...
    print("Extracting Vaikrá section...")
    vaikra_text = extract_vaikra_section(normalized_text)

    if not vaikra_text:
        print("Error: Could not extract Vaikrá section")
        sys.exit(1)

    print("Applying minimal formatting...")
    formatted_text = format_to_target_structure(vaikra_text)
    ...
    print("✓ Vaikrá extraction completed successfully!")
