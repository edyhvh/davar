#!/usr/bin/env python3
"""
Extract Melakim Bet (2 Kings) from tanaj.docx and create melajim_bet.md
...
def extract_melajim_bet_section(text: str) -> str:
    """Extract the Melajim Bet section from the full text."""
    lines = text.split('\n')
    print(f"Processing {len(lines)} lines...")

    # Find Melajim Bet start
    melajim_bet_start = -1
    for i, line in enumerate(lines):
        if line.strip() == '__MELAJIM BET \\(2 REYES\\)__ב מלכים':
            melajim_bet_start = i
            print(f"Found Melajim Bet start at line {i}")
            break

    if melajim_bet_start == -1:
        print("Error: Melajim Bet title not found")
        return ""

    # Find Melajim Bet end (next book)
    melajim_bet_end = -1
    for i in range(melajim_bet_start + 1, len(lines)):
        line = lines[i]
        ...
        # Stop at the main title of the next book (Yeshayahu/Isaiah)
        if line.strip() == '__IESHAIÁHU \\(ISAÍAS\\)__ ישעיהו':
            melajim_bet_end = i
            print(f"Found Melajim Bet end at line {i}: {line[:50]}...")
            break

    # If no next book found, take everything until footnotes start
    if melajim_bet_end == -1:
        for i in range(melajim_bet_start + 1, len(lines)):
    ...
    if melajim_bet_end == -1:
        ...
        melajim_bet_end = len(lines)

    # Extract Melajim Bet lines
    melajim_bet_lines = lines[melajim_bet_start:melajim_bet_end]

    # Find all footnote numbers used in Melajim Bet
    footnote_nums = set()
    melajim_bet_text = '\n'.join(melajim_bet_lines)
    # Find footnote references like [^1]
    for match in re.finditer(r'\[\^(\d+)\]', melajim_bet_text):
        footnote_nums.add(int(match.group(1)))

    print(f"Found {len(footnote_nums)} unique footnotes in Melajim Bet")

    # Extract footnote definitions that belong to Melajim Bet
    footnote_lines = []
    if footnote_nums:
        for i in range(melajim_bet_end, len(lines)):
            line = lines[i]
    ...
    return '\n'.join(melajim_bet_lines + footnote_lines)
...
def format_to_target_structure(text: str) -> str:
    ...
    while i < len(lines):
        ...
        if (line_length > 0 and line_length < 100 and
            ...
            has_title_patterns = (
                ...
                any(name in line_stripped for name in [
                    ...
                    'Iehóajaz', 'Iehóakash', 'Iehóakim', 'Tzedekiah',
                    'Iehoyakim', 'Tzedekiah', 'Tzidkiyah', 'Iehoyajin'
                ]) or
        ...
        if (line_length < 150 and line_length > 0 and '__' in line and
            ...
            # Check for individual book titles (like Melajim Bet)
            if line.strip() == '__MELAJIM BET \\(2 REYES\\)__ב מלכים':
                book_title_found = '__MELAJIM BET (2 REYES)*__ב מלכים*'
...
def main():
    ...
    output_file = "melajim_bet.md"

    if not os.path.exists(input_file):
        print(f"Error: Input file '{input_file}' does not exist.")
        sys.exit(1)

    print(f"Converting '{input_file}' to extract Melajim Bet section...")
    ...
    print("Extracting Melajim Bet section...")
    melajim_bet_text = extract_melajim_bet_section(normalized_text)

    if not melajim_bet_text:
        print("Error: Could not extract Melajim Bet section")
        sys.exit(1)

    print("Applying minimal formatting...")
    formatted_text = format_to_target_structure(melajim_bet_text)
    ...
    print("✓ Melajim Bet extraction completed successfully!")
