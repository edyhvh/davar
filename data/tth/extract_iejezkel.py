#!/usr/bin/env python3
"""
Extract Iejezkel (Ezekiel) from tanaj.docx and create iejezkel.md
...
def extract_iejezkel_section(text: str) -> str:
    """Extract the Iejezkel section from the full text."""
    lines = text.split('\n')
    ...
    for i, line in enumerate(lines):
        if line.strip() == '__IEJEZKEL \\(EZEQUIEL\\)__ יחזקאל':
            iejezkel_start = i
            print(f"Found Iejezkel start at line {i}")
            break

    if iejezkel_start == -1:
        print("Error: Iejezkel title not found")
        return ""

    # Find Iejezkel end (next book: Daniel)
    iejezkel_end = -1
    for i in range(iejezkel_start + 1, len(lines)):
        line = lines[i]
        ...
        # Stop at the main title of the next book (Hoshea/Oseas)
        if 'HOSHEA' in line_upper or 'HOSEA' in line_upper or 'הושע' in line:
            iejezkel_end = i
            print(f"Found Iejezkel end at line {i}: {line[:50]}...")
            break
    ...
    if iejezkel_end == -1:
        ...
        iejezkel_end = len(lines)

    # Extract Iejezkel lines
    iejezkel_lines = lines[iejezkel_start:iejezkel_end]

    # Find all footnote numbers used in Iejezkel
    footnote_nums = set()
    iejezkel_text = '\n'.join(iejezkel_lines)
    # Find footnote references like [^1]
    for match in re.finditer(r'\[\^(\d+)\]', iejezkel_text):
        footnote_nums.add(int(match.group(1)))

    print(f"Found {len(footnote_nums)} unique footnotes in Iejezkel")

    # Extract footnote definitions that belong to Iejezkel
    footnote_lines = []
    if footnote_nums:
        for i in range(iejezkel_end, len(lines)):
            line = lines[i]
    ...
    return '\n'.join(iejezkel_lines + footnote_lines)
...
def format_to_target_structure(text: str) -> str:
    ...
    while i < len(lines):
        ...
        if (line_length < 150 and line_length > 0 and '__' in line and
            ...
            # Check for individual book titles (like Iejezkel)
            if line.strip() == '__IEJEZKEL \\(EZEQUIEL\\)__ יחזקאל':
                book_title_found = '__IEJEZKEL (EZEQUIEL)*__יחזקאל*'
...
def main():
    ...
    output_file = "iejezkel.md"

    if not os.path.exists(input_file):
        print(f"Error: Input file '{input_file}' does not exist.")
        sys.exit(1)

    print(f"Converting '{input_file}' to extract Iejezkel section...")
    ...
    print("Extracting Iejezkel section...")
    iejezkel_text = extract_iejezkel_section(normalized_text)

    if not iejezkel_text:
        print("Error: Could not extract Iejezkel section")
        sys.exit(1)

    print("Applying minimal formatting...")
    formatted_text = format_to_target_structure(iejezkel_text)
    ...
    print("✓ Iejezkel extraction completed successfully!")
