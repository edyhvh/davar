#!/usr/bin/env python3
"""
Extract Melajim Alef (1 Kings) from tanaj.docx and create melajim_alef.md
...
def extract_melajim_alef_section(text: str) -> str:
    """Extract the Melajim Alef section from the full text."""
    lines = text.split('\n')
    print(f"Processing {len(lines)} lines...")

    # Find Melajim Alef start
    melajim_alef_start = -1
    for i, line in enumerate(lines):
        if line.strip() == '__MELAJIM ALEF \\(1 REYES\\)__א מלכים':
            melajim_alef_start = i
            print(f"Found Melajim Alef start at line {i}")
            break

    if melajim_alef_start == -1:
        print("Error: Melajim Alef title not found")
        return ""

    # Find Melajim Alef end (next book: MELAJIM BET/2 Kings)
    melajim_alef_end = -1
    for i in range(melajim_alef_start + 1, len(lines)):
        line = lines[i]
        ...
        # Stop at the main title of the next book (Melakim Bet/2 Kings)
        if line.strip() == '__MELAJIM BET \\(2 REYES\\)__ב מלכים':
            melajim_alef_end = i
            print(f"Found Melajim Alef end at line {i}")
            break

    # If no next book found, take everything until footnotes start
    if melajim_alef_end == -1:
        for i in range(melajim_alef_start + 1, len(lines)):
    ...
    if melajim_alef_end == -1:
        ...
        melajim_alef_end = len(lines)

    # Extract Melajim Alef lines
    melajim_alef_lines = lines[melajim_alef_start:melajim_alef_end]

    # Find all footnote numbers used in Melajim Alef
    footnote_nums = set()
    melajim_alef_text = '\n'.join(melajim_alef_lines)
    # Find footnote references like [^1]
    for match in re.finditer(r'\[\^(\d+)\]', melajim_alef_text):
        footnote_nums.add(int(match.group(1)))

    print(f"Found {len(footnote_nums)} unique footnotes in Melajim Alef")

    # Extract footnote definitions that belong to Melajim Alef
    footnote_lines = []
    if footnote_nums:
        for i in range(melajim_alef_end, len(lines)):
            line = lines[i]
    ...
    return '\n'.join(melajim_alef_lines + footnote_lines)
...
def format_to_target_structure(text: str) -> str:
    ...
    while i < len(lines):
        ...
        if (line_length > 0 and line_length < 100 and
            ...
            has_title_patterns = (
                ...
                any(name in line_stripped for name in [
                    'Shelomóh', 'David', 'Iehoshafat', 'Ajab', 'Elijah',
                    'Elishá', 'Iehoram', 'Amasiah', 'Oziahu', 'Iotam',
                    'Ajaz', 'Jizkiyah', 'Manasheh', 'Amón', 'Ioshiyah'
                ]) or
                ...
                (line_length < 50 and (
                    ...
                    any(word in line_stripped.lower() for word in ['rey', 'profeta', 'sacerdote', 'juez'])
                ))
            )
        ...
        if (line_length < 150 and line_length > 0 and '__' in line and
            ...
            # Check for individual book titles (like Melakim Alef)
            if line.strip() == '__MELAJIM ALEF \\(1 REYES\\)__א מלכים':
                book_title_found = '__MELAJIM ALEF (1 REYES)*__א מלכים*'
...
def main():
    ...
    output_file = "melajim_alef.md"

    if not os.path.exists(input_file):
        print(f"Error: Input file '{input_file}' does not exist.")
        sys.exit(1)

    print(f"Converting '{input_file}' to extract Melakim Alef section...")
    ...
    print("Extracting Melajim Alef section...")
    melajim_alef_text = extract_melajim_alef_section(normalized_text)

    if not melajim_alef_text:
        print("Error: Could not extract Melajim Alef section")
        sys.exit(1)

    print("Applying minimal formatting...")
    formatted_text = format_to_target_structure(melajim_alef_text)
    ...
    print("✓ Melajim Alef extraction completed successfully!")
