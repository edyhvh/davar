#!/usr/bin/env python3
"""
Extract Bamidbar (Numbers) from tanaj.docx and create bamidbar.md
Based on the optimized workflow from Bereshit, Shemot, and Vaikrá
"""
...
def extract_bamidbar_section(text: str) -> str:
    """Extract the Bamidbar section from the full text."""
    lines = text.split('\n')
    ...
    for i, line in enumerate(lines):
        if line.strip() == '__BAMIDBAR \\(NÚMEROS\\)__במדבר':
            bamidbar_start = i
            print(f"Found Bamidbar start at line {i}")
            break

    if bamidbar_start == -1:
        print("Error: Bamidbar title not found")
        return ""

    # Find Bamidbar end (next book: DEVÁRIM/Deuteronomy)
    bamidbar_end = -1
    for i in range(bamidbar_start + 1, len(lines)):
        ...
        # Stop at the main title of the next Torah book (Devárim/Deuteronomy)
        if line.strip() == '__DEVARIM \\(DEUTERONOMIO\\)__ דברים':
            bamidbar_end = i
            print(f"Found Bamidbar end at line {i}")
            break
    ...
    if bamidbar_end == -1:
        ...
        bamidbar_end = len(lines)

    # Extract Bamidbar lines
    bamidbar_lines = lines[bamidbar_start:bamidbar_end]

    # Find all footnote numbers used in Bamidbar
    footnote_nums = set()
    bamidbar_text = '\n'.join(bamidbar_lines)
    # Find footnote references like [^1]
    for match in re.finditer(r'\[\^(\d+)\]', bamidbar_text):
        footnote_nums.add(int(match.group(1)))

    print(f"Found {len(footnote_nums)} unique footnotes in Bamidbar")

    # Extract footnote definitions that belong to Bamidbar
    footnote_lines = []
    if footnote_nums:
        for i in range(bamidbar_end, len(lines)):
            line = lines[i]
    ...
    return '\n'.join(bamidbar_lines + footnote_lines)
...
def format_to_target_structure(text: str) -> str:
    ...
    while i < len(lines):
        ...
        if (line_length < 150 and line_length > 0 and '__' in line and
            ...
            # Check for individual book titles (like Bamidbar)
            if line.strip() == '__BAMIDBAR \\(NÚMEROS\\)__במדבר':
                book_title_found = '__BAMIDBAR (NÚMEROS)*__במדבר*'
...
def main():
    ...
    output_file = "bamidbar.md"

    if not os.path.exists(input_file):
        print(f"Error: Input file '{input_file}' does not exist.")
        sys.exit(1)

    print(f"Converting '{input_file}' to extract Bamidbar section...")
    ...
    print("Extracting Bamidbar section...")
    bamidbar_text = extract_bamidbar_section(normalized_text)

    if not bamidbar_text:
        print("Error: Could not extract Bamidbar section")
        sys.exit(1)

    print("Applying minimal formatting...")
    formatted_text = format_to_target_structure(bamidbar_text)
    ...
    print("✓ Bamidbar extraction completed successfully!")
