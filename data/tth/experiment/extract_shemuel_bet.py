#!/usr/bin/env python3
"""
Extract Shemuel Bet (2 Samuel) from tanaj.docx and create shemuel_bet.md
...
def extract_shemuel_bet_section(text: str) -> str:
    """Extract the Shemuel Bet section from the full text."""
    lines = text.split('\n')
    print(f"Processing {len(lines)} lines...")

    # Find Shemuel Bet start
    shemuel_bet_start = -1
    for i, line in enumerate(lines):
        if line.strip() == '__SHEMUEL BET \\(2 SAMUEL\\)__ ב שמואל':
            shemuel_bet_start = i
            print(f"Found Shemuel Bet start at line {i}")
            break

    if shemuel_bet_start == -1:
        print("Error: Shemuel Bet title not found")
        return ""

    # Find Shemuel Bet end (next book: MELAKIM ALEF/1 Kings)
    shemuel_bet_end = -1
    for i in range(shemuel_bet_start + 1, len(lines)):
        line = lines[i]
        ...
        # Stop at the main title of the next book (Melakim Alef/1 Kings)
        if line.strip() == '__MELAJIM ALEF \\(1 REYES\\)__א מלכים':
            shemuel_bet_end = i
            print(f"Found Shemuel Bet end at line {i}")
            break

    # If no next book found, take everything until footnotes start
    if shemuel_bet_end == -1:
        for i in range(shemuel_bet_start + 1, len(lines)):
    ...
    if shemuel_bet_end == -1:
        ...
        shemuel_bet_end = len(lines)

    # Extract Shemuel Bet lines
    shemuel_bet_lines = lines[shemuel_bet_start:shemuel_bet_end]

    # Find all footnote numbers used in Shemuel Bet
    footnote_nums = set()
    shemuel_bet_text = '\n'.join(shemuel_bet_lines)
    # Find footnote references like [^1]
    for match in re.finditer(r'\[\^(\d+)\]', shemuel_bet_text):
        footnote_nums.add(int(match.group(1)))

    print(f"Found {len(footnote_nums)} unique footnotes in Shemuel Bet")

    # Extract footnote definitions that belong to Shemuel Bet
    footnote_lines = []
    if footnote_nums:
        for i in range(shemuel_bet_end, len(lines)):
            line = lines[i]
    ...
    return '\n'.join(shemuel_bet_lines + footnote_lines)
...
def format_to_target_structure(text: str) -> str:
    ...
    while i < len(lines):
        ...
        if (line_length < 150 and line_length > 0 and '__' in line and
            ...
            # Check for individual book titles (like Shemuel Bet)
            if line.strip() == '__SHEMUEL BET \\(2 SAMUEL\\)__ ב שמואל':
                book_title_found = '__SHEMUEL BET (2 SAMUEL)*__ב שמואל*'
...
def main():
    ...
    output_file = "shemuel_bet.md"

    if not os.path.exists(input_file):
        print(f"Error: Input file '{input_file}' does not exist.")
        sys.exit(1)

    print(f"Converting '{input_file}' to extract Shemuel Bet section...")
    ...
    print("Extracting Shemuel Bet section...")
    shemuel_bet_text = extract_shemuel_bet_section(normalized_text)

    if not shemuel_bet_text:
        print("Error: Could not extract Shemuel Bet section")
        sys.exit(1)

    print("Applying minimal formatting...")
    formatted_text = format_to_target_structure(shemuel_bet_text)
    ...
    print("✓ Shemuel Bet extraction completed successfully!")
