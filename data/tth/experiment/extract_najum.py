#!/usr/bin/env python3
"""
Extract Najum (Nahum) from tanaj.docx and create najum.md
...
def extract_najum_section(text: str) -> str:
    """Extract the Najum section from the full text."""
    lines = text.split('\n')
    ...
    for i, line in enumerate(lines):
        if line.strip() == '__NAJUM \\(NAHÚM\\)__ נחום':
            najum_start = i
            print(f"Found Najum start at line {i}")
            break

    if najum_start == -1:
        print("Error: Najum title not found")
        return ""

    # Find Najum end (next book: Jabakuk)
    najum_end = -1
    for i in range(najum_start + 1, len(lines)):
        line = lines[i]
        ...
        # Stop at the main title of the next book (Jabakuk)
        if 'JABAKUK' in line_upper or 'HABAKUK' in line_upper or 'חבקוק' in line or 'JABAKUQ' in line_upper:
            najum_end = i
            print(f"Found Najum end at line {i}: {line[:50]}...")
            break
    ...
    # Extract Najum lines
    najum_lines = lines[najum_start:najum_end]

    # Find all footnote numbers used in Najum
    footnote_nums = set()
    najum_text = '\n'.join(najum_lines)
    # Find footnote references like [^1]
    for match in re.finditer(r'\[\^(\d+)\]', najum_text):
        footnote_nums.add(int(match.group(1)))

    print(f"Found {len(footnote_nums)} unique footnotes in Najum")

    # Extract footnote definitions that belong to Najum
    footnote_lines = []
    if footnote_nums:
        for i in range(najum_end, len(lines)):
            line = lines[i]
    ...
    return '\n'.join(najum_lines + footnote_lines)
...
def format_to_target_structure(text: str) -> str:
    ...
    while i < len(lines):
        ...
        if (line_length < 150 and line_length > 0 and '__' in line and
            ...
            # Check for individual book titles (like Najum)
            if line.strip() == '__NAJUM \\(NAHÚM\\)__ נחום':
                book_title_found = '__NAJUM (NAHÚM)*__ נחום*'
...
def main():
    ...
    output_file = "najum.md"

    if not os.path.exists(input_file):
        print(f"Error: Input file '{input_file}' does not exist.")
        sys.exit(1)

    print(f"Converting '{input_file}' to extract Najum section...")
    ...
    print("Extracting Najum section...")
    najum_text = extract_najum_section(normalized_text)

    if not najum_text:
        print("Error: Could not extract Najum section")
        sys.exit(1)

    print("Applying minimal formatting...")
    formatted_text = format_to_target_structure(najum_text)
    ...
    print("✓ Najum extraction completed successfully!")
