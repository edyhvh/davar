#!/usr/bin/env python3
"""
Extract Hoshea (Hosea) from tanaj.docx and create hoshea.md
...
def extract_hoshea_section(text: str) -> str:
    """Extract the Hoshea section from the full text."""
    lines = text.split('\n')
    ...
    for i, line in enumerate(lines):
        if line.strip() == '__HOSHEA \\(OSEAS\\)__הושע':
            hoshea_start = i
            print(f"Found Hoshea start at line {i}")
            break

    if hoshea_start == -1:
        print("Error: Hoshea title not found")
        return ""

    # Find Hoshea end (next book: Ioel/Joel)
    hoshea_end = -1
    for i in range(hoshea_start + 1, len(lines)):
        line = lines[i]
        ...
        # Stop at the main title of the next book (Ioel/Joel)
        if 'IOEL' in line_upper or 'JOEL' in line_upper or 'יואל' in line:
            hoshea_end = i
            print(f"Found Hoshea end at line {i}: {line[:50]}...")
            break
    ...
    if hoshea_end == -1:
        ...
        hoshea_end = len(lines)

    # Extract Hoshea lines
    hoshea_lines = lines[hoshea_start:hoshea_end]

    # Find all footnote numbers used in Hoshea
    footnote_nums = set()
    hoshea_text = '\n'.join(hoshea_lines)
    # Find footnote references like [^1]
    for match in re.finditer(r'\[\^(\d+)\]', hoshea_text):
        footnote_nums.add(int(match.group(1)))

    print(f"Found {len(footnote_nums)} unique footnotes in Hoshea")

    # Extract footnote definitions that belong to Hoshea
    footnote_lines = []
    if footnote_nums:
        for i in range(hoshea_end, len(lines)):
            line = lines[i]
    ...
    return '\n'.join(hoshea_lines + footnote_lines)
...
def format_to_target_structure(text: str) -> str:
    ...
    while i < len(lines):
        ...
        if (line_length < 150 and line_length > 0 and '__' in line and
            ...
            # Check for individual book titles (like Hoshea)
            if line.strip() == '__HOSHEA \\(OSEAS\\)__הושע':
                book_title_found = '__HOSHEA (OSEAS)*__הושע*'
...
def main():
    ...
    output_file = "hoshea.md"

    if not os.path.exists(input_file):
        print(f"Error: Input file '{input_file}' does not exist.")
        sys.exit(1)

    print(f"Converting '{input_file}' to extract Hoshea section...")
    ...
    print("Extracting Hoshea section...")
    hoshea_text = extract_hoshea_section(normalized_text)

    if not hoshea_text:
        print("Error: Could not extract Hoshea section")
        sys.exit(1)

    print("Applying minimal formatting...")
    formatted_text = format_to_target_structure(hoshea_text)
    ...
    print("✓ Hoshea extraction completed successfully!")
