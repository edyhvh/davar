#!/usr/bin/env python3
"""
Extract Ioel (Joel) from tanaj.docx and create ioel.md
...
def extract_ioel_section(text: str) -> str:
    """Extract the Ioel section from the full text."""
    lines = text.split('\n')
    ...
    for i, line in enumerate(lines):
        if line.strip() == '__IOEL \\(JOEL\\)__יואל':
            ioel_start = i
            print(f"Found Ioel start at line {i}")
            break

    if ioel_start == -1:
        print("Error: Ioel title not found")
        return ""

    # Find Ioel end (next book: Amos)
    ioel_end = -1
    for i in range(ioel_start + 1, len(lines)):
        line = lines[i]
        ...
        # Stop at the main title of the next book (Amos)
        if 'AMOS' in line_upper or 'עמוס' in line:
            ioel_end = i
            print(f"Found Ioel end at line {i}: {line[:50]}...")
            break
    ...
    # Extract Ioel lines
    ioel_lines = lines[ioel_start:ioel_end]

    # Find all footnote numbers used in Ioel
    footnote_nums = set()
    ioel_text = '\n'.join(ioel_lines)
    # Find footnote references like [^1]
    for match in re.finditer(r'\[\^(\d+)\]', ioel_text):
        footnote_nums.add(int(match.group(1)))

    print(f"Found {len(footnote_nums)} unique footnotes in Ioel")

    # Extract footnote definitions that belong to Ioel
    footnote_lines = []
    if footnote_nums:
        for i in range(ioel_end, len(lines)):
            line = lines[i]
    ...
    return '\n'.join(ioel_lines + footnote_lines)
...
def format_to_target_structure(text: str) -> str:
    ...
    while i < len(lines):
        ...
        if (line_length < 150 and line_length > 0 and '__' in line and
            ...
            # Check for individual book titles (like Ioel)
            if line.strip() == '__IOEL \\(JOEL\\)__יואל':
                book_title_found = '__IOEL (JOEL)*__יואל*'
...
def main():
    ...
    print(f"Converting '{input_file}' to extract Ioel section...")
    ...
    print("Extracting Ioel section...")
    ioel_text = extract_ioel_section(normalized_text)

    if not ioel_text:
        print("Error: Could not extract Ioel section")
        sys.exit(1)

    print("Applying minimal formatting...")
    formatted_text = format_to_target_structure(ioel_text)
    ...
    print("✓ Ioel extraction completed successfully!")
