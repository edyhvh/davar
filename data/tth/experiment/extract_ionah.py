#!/usr/bin/env python3
"""
Extract Ionah (Jonah) from tanaj.docx and create ionah.md
...
def extract_ionah_section(text: str) -> str:
    """Extract the Ionah section from the full text."""
    lines = text.split('\n')
    ...
    for i, line in enumerate(lines):
        if line.strip() == '__IONÁH \\(JONÁS\\)__ יונה':
            ionah_start = i
            print(f"Found Ionah start at line {i}")
            break

    if ionah_start == -1:
        print("Error: Ionah title not found")
        return ""

    # Find Ionah end (next book: Micah)
    ionah_end = -1
    for i in range(ionah_start + 1, len(lines)):
        line = lines[i]
        ...
        # Stop at the main title of the next book (Micah)
        if 'MICAH' in line_upper or 'MIQUEAS' in line_upper or 'מיכה' in line:
            ionah_end = i
            print(f"Found Ionah end at line {i}: {line[:50]}...")
            break
    ...
    # Extract Ionah lines
    ionah_lines = lines[ionah_start:ionah_end]

    # Find all footnote numbers used in Ionah
    footnote_nums = set()
    ionah_text = '\n'.join(ionah_lines)
    # Find footnote references like [^1]
    for match in re.finditer(r'\[\^(\d+)\]', ionah_text):
        footnote_nums.add(int(match.group(1)))

    print(f"Found {len(footnote_nums)} unique footnotes in Ionah")

    # Extract footnote definitions that belong to Ionah
    footnote_lines = []
    if footnote_nums:
        for i in range(ionah_end, len(lines)):
            line = lines[i]
    ...
    return '\n'.join(ionah_lines + footnote_lines)
...
def format_to_target_structure(text: str) -> str:
    ...
    while i < len(lines):
        ...
        if (line_length < 150 and line_length > 0 and '__' in line and
            ...
            # Check for individual book titles (like Ionah)
            if line.strip() == '__IONÁH \\(JONÁS\\)__ יונה':
                book_title_found = '__IONÁH (JONÁS)*__ יונה*'
...
def main():
    ...
    output_file = "ionah.md"

    if not os.path.exists(input_file):
        print(f"Error: Input file '{input_file}' does not exist.")
        sys.exit(1)

    print(f"Converting '{input_file}' to extract Ionah section...")
    ...
    print("Extracting Ionah section...")
    ionah_text = extract_ionah_section(normalized_text)

    if not ionah_text:
        print("Error: Could not extract Ionah section")
        sys.exit(1)

    print("Applying minimal formatting...")
    formatted_text = format_to_target_structure(ionah_text)
    ...
    print("✓ Ionah extraction completed successfully!")
