#!/usr/bin/env python3
"""
Extract Tzefaniah (Zephaniah) from tanaj.docx and create tzefaniah.md
...
def extract_tzefaniah_section(text: str) -> str:
    """Extract the Tzefaniah section from the full text."""
    lines = text.split('\n')
    print(f"Processing {len(lines)} lines...")

    # Find Tzefaniah start
    tzefaniah_start = -1
    for i, line in enumerate(lines):
        if line.strip() == '__TZEFANIÁH \\(SOFONÍAS\\)__ צפניה':
            tzefaniah_start = i
            print(f"Found Tzefaniah start at line {i}")
            break

    if tzefaniah_start == -1:
        print("Error: Tzefaniah title not found")
        return ""

    # Find Tzefaniah end (next book: Jagai)
    tzefaniah_end = -1
    for i in range(tzefaniah_start + 1, len(lines)):
        line = lines[i]
        ...
        # Stop at the main title of the next book (Jagai)
        if 'JAGAI' in line_upper or 'HAGGAI' in line_upper or 'JAGGAI' in line_upper or 'חגי' in line:
            tzefaniah_end = i
            print(f"Found Tzefaniah end at line {i}: {line[:50]}...")
            break
    ...
    if tzefaniah_end == -1:
        ...
        tzefaniah_end = len(lines)

    # Extract Tzefaniah lines
    tzefaniah_lines = lines[tzefaniah_start:tzefaniah_end]

    # Find all footnote numbers used in Tzefaniah
    footnote_nums = set()
    tzefaniah_text = '\n'.join(tzefaniah_lines)
    # Find footnote references like [^1]
    for match in re.finditer(r'\[\^(\d+)\]', tzefaniah_text):
        footnote_nums.add(int(match.group(1)))

    print(f"Found {len(footnote_nums)} unique footnotes in Tzefaniah")

    # Extract footnote definitions that belong to Tzefaniah
    footnote_lines = []
    if footnote_nums:
        for i in range(tzefaniah_end, len(lines)):
            line = lines[i]
    ...
    return '\n'.join(tzefaniah_lines + footnote_lines)
...
def format_to_target_structure(text: str) -> str:
    ...
    while i < len(lines):
        ...
        if (line_length > 0 and line_length < 100 and
            ...
            has_title_patterns = (
                ...
                any(name in line_stripped for name in [
                    'Iejezkel', 'Yechezquel', 'Ezekiel', 'David', 'Shelomoh', 'Abraham',
                    'Itzhak', 'Iaacob', 'Moshéh', 'Aharón', 'Yirmeyahu',
                    'Daniél', 'Hoshea', 'Ioel', 'Amós',
        ...
        if (line_length < 150 and line_length > 0 and '__' in line and
            ...
            # Check for individual book titles (like Tzefaniah)
            if line.strip() == '__TZEFANIÁH \\(SOFONÍAS\\)__ צפניה':
                book_title_found = '__TZEFANIÁH (SOFONÍAS)*__ צפניה*'
...
def main():
    ...
    output_file = "tzefaniah.md"

    if not os.path.exists(input_file):
        print(f"Error: Input file '{input_file}' does not exist.")
        sys.exit(1)

    print(f"Converting '{input_file}' to extract Tzefaniah section...")
    ...
    print("Extracting Tzefaniah section...")
    tzefaniah_text = extract_tzefaniah_section(normalized_text)

    if not tzefaniah_text:
        print("Error: Could not extract Tzefaniah section")
        sys.exit(1)

    print("Applying minimal formatting...")
    formatted_text = format_to_target_structure(tzefaniah_text)
    ...
    print("✓ Tzefaniah extraction completed successfully!")
