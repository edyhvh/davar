#!/usr/bin/env python3
"""
Extract Jagai (Haggai) from tanaj.docx and create jagai.md
...
def extract_jagai_section(text: str) -> str:
    """Extract the Jagai section from the full text."""
    lines = text.split('\n')
    ...
    for i, line in enumerate(lines):
        if line.strip() == '__JAGAI \\(HAGEO\\)__חגי':
            jagai_start = i
            print(f"Found Jagai start at line {i}")
            break

    if jagai_start == -1:
        print("Error: Jagai title not found")
        return ""

    # Find Jagai end (next book: Zekariah/Zechariah)
    jagai_end = -1
    for i in range(jagai_start + 1, len(lines)):
        line = lines[i]
        ...
        # Stop at the main title of the next book (Zekariah/Zechariah)
        if 'ZEJARIAH' in line_upper or 'ZEKARIAH' in line_upper or 'ZACARÍAS' in line_upper or 'זכריה' in line:
            jagai_end = i
            print(f"Found Jagai end at line {i}: {line[:50]}...")
            break
    ...
    # Extract Jagai lines
    jagai_lines = lines[jagai_start:jagai_end]

    # Find all footnote numbers used in Jagai
    footnote_nums = set()
    jagai_text = '\n'.join(jagai_lines)
    # Find footnote references like [^1]
    for match in re.finditer(r'\[\^(\d+)\]', jagai_text):
        footnote_nums.add(int(match.group(1)))

    print(f"Found {len(footnote_nums)} unique footnotes in Jagai")

    # Extract footnote definitions that belong to Jagai
    footnote_lines = []
    if footnote_nums:
        for i in range(jagai_end, len(lines)):
            line = lines[i]
    ...
    return '\n'.join(jagai_lines + footnote_lines)
...
def format_to_target_structure(text: str) -> str:
    ...
    while i < len(lines):
        ...
        if (line_length < 150 and line_length > 0 and '__' in line and
            ...
            # Check for individual book titles (like Jagai)
            if line.strip() == '__JAGAI \\(HAGEO\\)__חגי':
                book_title_found = '__JAGAI (HAGEO)*__חגי*'
...
def main():
    ...
    output_file = "jagai.md"

    if not os.path.exists(input_file):
        print(f"Error: Input file '{input_file}' does not exist.")
        sys.exit(1)

    print(f"Converting '{input_file}' to extract Jagai section...")
    ...
    print("Extracting Jagai section...")
    jagai_text = extract_jagai_section(normalized_text)

    if not jagai_text:
        print("Error: Could not extract Jagai section")
        sys.exit(1)

    print("Applying minimal formatting...")
    formatted_text = format_to_target_structure(jagai_text)
    ...
    print("✓ Jagai extraction completed successfully!")
