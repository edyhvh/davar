#!/usr/bin/env python3
"""
Extract Micah (Micah) from tanaj.docx and create micah.md
...
def extract_micah_section(text: str) -> str:
    """Extract the Micah section from the full text."""
    lines = text.split('\n')
    ...
    for i, line in enumerate(lines):
        if line.strip() == '__MICÁH \\(MIQUEAS\\)__ מיכה':
            micah_start = i
            print(f"Found Micah start at line {i}")
            break

    if micah_start == -1:
        print("Error: Micah title not found")
        return ""

    # Find Micah end (next book: Najum)
    micah_end = -1
    for i in range(micah_start + 1, len(lines)):
        line = lines[i]
        ...
        # Stop at the main title of the next book (Najum)
        if 'NAJUM' in line_upper or 'NAHUM' in line_upper or 'נחום' in line:
            micah_end = i
            print(f"Found Micah end at line {i}: {line[:50]}...")
            break
    ...
    # Extract Micah lines
    micah_lines = lines[micah_start:micah_end]

    # Find all footnote numbers used in Micah
    footnote_nums = set()
    micah_text = '\n'.join(micah_lines)
    # Find footnote references like [^1]
    for match in re.finditer(r'\[\^(\d+)\]', micah_text):
        footnote_nums.add(int(match.group(1)))

    print(f"Found {len(footnote_nums)} unique footnotes in Micah")

    # Extract footnote definitions that belong to Micah
    footnote_lines = []
    if footnote_nums:
        for i in range(micah_end, len(lines)):
            line = lines[i]
    ...
    return '\n'.join(micah_lines + footnote_lines)
...
def format_to_target_structure(text: str) -> str:
    ...
    while i < len(lines):
        ...
        if (line_length < 150 and line_length > 0 and '__' in line and
            ...
            # Check for individual book titles (like Micah)
            if line.strip() == '__MICÁH \\(MIQUEAS\\)__ מיכה':
                book_title_found = '__MICÁH (MIQUEAS)*__ מיכה*'
...
def main():
    ...
    output_file = "micah.md"

    if not os.path.exists(input_file):
        print(f"Error: Input file '{input_file}' does not exist.")
        sys.exit(1)

    print(f"Converting '{input_file}' to extract Micah section...")
    ...
    print("Extracting Micah section...")
    micah_text = extract_micah_section(normalized_text)

    if not micah_text:
        print("Error: Could not extract Micah section")
        sys.exit(1)

    print("Applying minimal formatting...")
    formatted_text = format_to_target_structure(micah_text)
    ...
    print("✓ Micah extraction completed successfully!")
