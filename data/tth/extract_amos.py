#!/usr/bin/env python3
"""
Extract Amos (Amos) from tanaj.docx and create amos.md
...
def extract_amos_section(text: str) -> str:
    """Extract the Amos section from the full text."""
    lines = text.split('\n')
    ...
    for i, line in enumerate(lines):
        if line.strip() == '__AMÓS__ עמוס':
            amos_start = i
            print(f"Found Amos start at line {i}")
            break

    if amos_start == -1:
        print("Error: Amos title not found")
        return ""

    # Find Amos end (next book: Obadiah)
    amos_end = -1
    for i in range(amos_start + 1, len(lines)):
        line = lines[i]
        ...
        # Stop at the main title of the next book (Obadiah)
        if 'OBADIAH' in line_upper or 'OVADIAH' in line_upper or 'עבדיה' in line or 'OBADÍA' in line_upper:
            amos_end = i
            print(f"Found Amos end at line {i}: {line[:50]}...")
            break
    ...
    # Extract Amos lines
    amos_lines = lines[amos_start:amos_end]

    # Find all footnote numbers used in Amos
    footnote_nums = set()
    amos_text = '\n'.join(amos_lines)
    # Find footnote references like [^1]
    for match in re.finditer(r'\[\^(\d+)\]', amos_text):
        footnote_nums.add(int(match.group(1)))

    print(f"Found {len(footnote_nums)} unique footnotes in Amos")

    # Extract footnote definitions that belong to Amos
    footnote_lines = []
    if footnote_nums:
        for i in range(amos_end, len(lines)):
            line = lines[i]
    ...
    return '\n'.join(amos_lines + footnote_lines)
...
def format_to_target_structure(text: str) -> str:
    ...
    while i < len(lines):
        ...
        if (line_length < 150 and line_length > 0 and '__' in line and
            ...
            # Check for individual book titles (like Amos)
            if line.strip() == '__AMÓS__ עמוס':
                book_title_found = '__AMÓS__ עמוס*'
...
def main():
    ...
    print(f"Converting '{input_file}' to extract Amos section...")
    ...
    print("Extracting Amos section...")
    amos_text = extract_amos_section(normalized_text)

    if not amos_text:
        print("Error: Could not extract Amos section")
        sys.exit(1)

    print("Applying minimal formatting...")
    formatted_text = format_to_target_structure(amos_text)
    ...
    print("✓ Amos extraction completed successfully!")
