#!/usr/bin/env python3
"""
Extract Iehoshúa (Joshua) from tanaj.docx and create iehosua.md
Based on the optimized workflow from the Torah books
"""
...
def extract_iehosua_section(text: str) -> str:
    """Extract the Iehoshúa section from the full text."""
    lines = text.split('\n')
    ...
    for i, line in enumerate(lines):
        if line.strip() == '__NEVIÍM \\- IEHOSHÚA \\(JOSUÉ\\)__ יהושע':
            iehosua_start = i
            print(f"Found Iehoshúa start at line {i}")
            break

    if iehosua_start == -1:
        print("Error: Iehoshúa title not found")
        return ""

    # Find Iehoshúa end (next book: SHOFTÍM/Judges)
    iehosua_end = -1
    for i in range(iehosua_start + 1, len(lines)):
        line = lines[i]
        ...
        # Stop at the main title of the next book (Shoftím/Judges)
        if line.strip() == '__SHOFTÍM \\(JUECES\\)__ שפטים':
            iehosua_end = i
            print(f"Found Iehoshúa end at line {i}")
            break
    ...
    if iehosua_end == -1:
        ...
        iehosua_end = len(lines)

    # Extract Iehoshúa lines
    iehosua_lines = lines[iehosua_start:iehosua_end]

    # Find all footnote numbers used in Iehoshúa
    footnote_nums = set()
    iehosua_text = '\n'.join(iehosua_lines)
    # Find footnote references like [^1]
    for match in re.finditer(r'\[\^(\d+)\]', iehosua_text):
        footnote_nums.add(int(match.group(1)))

    print(f"Found {len(footnote_nums)} unique footnotes in Iehoshúa")

    # Extract footnote definitions that belong to Iehoshúa
    footnote_lines = []
    if footnote_nums:
        for i in range(iehosua_end, len(lines)):
            line = lines[i]
    ...
    return '\n'.join(iehosua_lines + footnote_lines)
...
def format_to_target_structure(text: str) -> str:
    ...
    while i < len(lines):
        ...
        if (line_length < 150 and line_length > 0 and '__' in line and
            ...
            # Check for individual book titles (like Iehoshúa)
            if line.strip() == '__NEVIÍM \\- IEHOSHÚA \\(JOSUÉ\\)__ יהושע':
                book_title_found = '__NEVIÍM - IEHOSHÚA (JOSUÉ)*__יהושע*'
...
def main():
    ...
    output_file = "iehosua.md"

    if not os.path.exists(input_file):
        print(f"Error: Input file '{input_file}' does not exist.")
        sys.exit(1)

    print(f"Converting '{input_file}' to extract Iehoshúa section...")
    ...
    print("Extracting Iehoshúa section...")
    iehosua_text = extract_iehosua_section(normalized_text)

    if not iehosua_text:
        print("Error: Could not extract Iehoshúa section")
        sys.exit(1)

    print("Applying minimal formatting...")
    formatted_text = format_to_target_structure(iehosua_text)
    ...
    print("✓ Iehoshúa extraction completed successfully!")
