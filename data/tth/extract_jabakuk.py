#!/usr/bin/env python3
"""
Extract Jabakuk (Habakkuk) from tanaj.docx and create jabakuk.md
...
def extract_jabakuk_section(text: str) -> str:
    """Extract the Jabakuk section from the full text."""
    lines = text.split('\n')
    ...
    for i, line in enumerate(lines):
        if line.strip() == '__JABAKUK \\(HABACUC\\)__ חבקוק':
            jabakuk_start = i
            print(f"Found Jabakuk start at line {i}")
            break

    if jabakuk_start == -1:
        print("Error: Jabakuk title not found")
        return ""

    # Find Jabakuk end (next book: Tzefaniah)
    jabakuk_end = -1
    for i in range(jabakuk_start + 1, len(lines)):
        line = lines[i]
        ...
        # Stop at the main title of the next book (Tzefaniah)
        if 'TZEfANIAH' in line_upper or 'ZEFANIAH' in line_upper or 'TZEfANIA' in line_upper or 'צפניה' in line:
            jabakuk_end = i
            print(f"Found Jabakuk end at line {i}: {line[:50]}...")
            break
    ...
    if jabakuk_end == -1:
        ...
        jabakuk_end = len(lines)

    # Extract Jabakuk lines
    jabakuk_lines = lines[jabakuk_start:jabakuk_end]

    # Find all footnote numbers used in Jabakuk
    footnote_nums = set()
    jabakuk_text = '\n'.join(jabakuk_lines)
    # Find footnote references like [^1]
    for match in re.finditer(r'\[\^(\d+)\]', jabakuk_text):
        footnote_nums.add(int(match.group(1)))

    print(f"Found {len(footnote_nums)} unique footnotes in Jabakuk")

    # Extract footnote definitions that belong to Jabakuk
    footnote_lines = []
    if footnote_nums:
        for i in range(jabakuk_end, len(lines)):
            line = lines[i]
    ...
    return '\n'.join(jabakuk_lines + footnote_lines)
...
def format_to_target_structure(text: str) -> str:
    ...
    while i < len(lines):
        ...
        if (line_length < 150 and line_length > 0 and '__' in line and
            ...
            # Check for individual book titles (like Jabakuk)
            if line.strip() == '__JABAKUK \\(HABACUC\\)__ חבקוק':
                book_title_found = '__JABAKUK (HABACUC)*__ חבקוק*'
...
def main():
    ...
    output_file = "jabakuk.md"

    if not os.path.exists(input_file):
        print(f"Error: Input file '{input_file}' does not exist.")
        sys.exit(1)

    print(f"Converting '{input_file}' to extract Jabakuk section...")
    ...
    print("Extracting Jabakuk section...")
    jabakuk_text = extract_jabakuk_section(normalized_text)

    if not jabakuk_text:
        print("Error: Could not extract Jabakuk section")
        sys.exit(1)

    print("Applying minimal formatting...")
    formatted_text = format_to_target_structure(jabakuk_text)
    ...
    print("✓ Jabakuk extraction completed successfully!")
