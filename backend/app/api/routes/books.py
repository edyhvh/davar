...
async def get_books(
    section: Optional[BookSection] = Query(None, description="Filter by book section"),
    ...
    """
    try:
        books = books_service.get_all_books(section.value if section else None)
        return books
...
async def get_chapter_verses(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error retrieving verse count: {str(e)}")