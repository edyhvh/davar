...
class WordMatcher:
    ...
    def match_word(self, word: str) -> Dict:
        ...
        """
        # #region agent log
        debug_log('word_matcher.py:match_word',
                  'Starting word processing', {'original_word': word}, 'A')
        # #endregion
        ...
        # #region agent log
        debug_log('word_matcher.py:match_word', 'After cleaning and normalization', {
                  'cleaned_word': word, 'normalized_word': normalized_word}, 'A')
        # #endregion

        # STEP 1: Try whole word first (EXPANDED)
        # Try multiple lookup strategies for compound words
        # #region agent log
        debug_log('word_matcher.py:match_word', 'STEP 1: Whole word lookup', {
                  'normalized_word': normalized_word}, 'A')
        # #endregion

        strong_number = self.loader.get_strong_number(normalized_word)
        # #region agent log
        debug_log('word_matcher.py:match_word', 'STEP 1: Normalized lookup result', {
                  'normalized_word': normalized_word, 'strong_number': strong_number}, 'A')
        # #endregion
        ...
        if not strong_number:
            ...
            for prefix_char in ['ל', 'ב', 'כ', 'מ']:
                if normalized_word.startswith(prefix_char):
                    ...
                    strong_number = self.loader.get_strong_number(
                        without_prefix)
                    if strong_number:
                        # Found it! Return with single prefix
                        # #region agent log
                        debug_log('word_matcher.py:match_word', 'STEP 1: Found with prefix stripped', {
                                  'without_prefix': without_prefix, 'strong_number': strong_number}, 'A')
                        # #endregion
        ...
        if not strong_number:
            ...
            # strip nikud only, keep finals
            preserved_finals = strip_nikud(word)
            # #region agent log
            debug_log('word_matcher.py:match_word', 'STEP 1: Trying preserved finals', {
                      'preserved_finals': preserved_finals}, 'A')
            # #endregion

            strong_number = self.loader.get_strong_number(preserved_finals)
            # #region agent log
            debug_log('word_matcher.py:match_word', 'STEP 1: Preserved finals lookup result', {
                      'preserved_finals': preserved_finals, 'strong_number': strong_number}, 'A')
            # #endregion

        if strong_number:
            # #region agent log
            debug_log('word_matcher.py:match_word', 'STEP 1: Whole word match found', {
                      'strong_number': strong_number}, 'A')
            # #endregion
        ...
        # #region agent log
        debug_log('word_matcher.py:match_word',
                  'STEP 2: Starting prefix stripping', {}, 'B')
        # #endregion

        prefixes, stem = self.prefix_detector.identify_prefixes(word)

        # #region agent log
        debug_log('word_matcher.py:match_word', 'STEP 2: Prefix stripping result', {
                  'prefixes': prefixes, 'stem': stem}, 'B')
        # #endregion
        ...
        if prefixes and stem:
            ...
            if stem_clean in ['ו', 'י', 'ה', 'ם', 'ן', 'נו', 'כם'] or stem_clean == '':
                ...
                # #region agent log
                debug_log('word_matcher.py:match_word', 'STEP 4: Composite preposition match found', {
                          'prefixes': prefixes, 'suffix': suffix}, 'D')
                # #endregion
        ...
        # #region agent log
        debug_log('word_matcher.py:match_word', 'STEP 2: Stem validation', {
                  'stem': stem, 'stem_valid': stem_valid}, 'B')
        # #endregion
        ...
        if not stem_valid:
            ...
            # #region agent log
            debug_log('word_matcher.py:match_word',
                      'STEP 2: Invalid stem, returning null', {'stem': stem}, 'B')
            # #endregion
            self._log_unmatched(
                word, stem, "invalid stem after prefix removal")
            return self.formatter.format_word_result(word, None, [], None)

        normalized_stem = normalize_for_matching(stem)
        # #region agent log
        debug_log('word_matcher.py:match_word', 'STEP 2: Normalized stem for lookup', {
                  'normalized_stem': normalized_stem}, 'B')
        # #endregion

        strong_number = self.loader.get_strong_number(normalized_stem)
        # #region agent log
        debug_log('word_matcher.py:match_word', 'STEP 2: Stem lookup result', {
                  'normalized_stem': normalized_stem, 'strong_number': strong_number}, 'B')
        # #endregion

        if strong_number:
            # #region agent log
            debug_log('word_matcher.py:match_word', 'STEP 2: Prefix stripping match found', {
                      'strong_number': strong_number, 'prefixes': prefixes}, 'B')
            # #endregion
        ...
        # #region agent log
        debug_log('word_matcher.py:match_word', 'STEP 3: Starting suffix stripping', {
                  'normalized_stem': normalized_stem}, 'C')
        # #endregion
        ...
        # #region agent log
        debug_log('word_matcher.py:match_word', 'STEP 3: Suffix stripping result', {
                  'stem_with_finals': stem_with_finals, 'stem_no_suffix': stem_no_suffix, 'suffix_id': suffix_id}, 'C')
        # #endregion
        ...
        if suffix_id:
            ...
            # #region agent log
            debug_log('word_matcher.py:match_word', 'STEP 3: Normalized stem lookup result', {
                      'stem_normalized': stem_normalized, 'strong_number': strong_number}, 'C')
            # #endregion
            if strong_number:
                # #region agent log
                debug_log('word_matcher.py:match_word', 'STEP 3: Suffix stripping match found', {
                          'strong_number': strong_number, 'prefixes': prefixes, 'suffix_id': suffix_id}, 'C')
                # #endregion
                return self.formatter.format_word_result(word, strong_number, prefixes, suffix_id)

        # All strategies failed
        # #region agent log
        debug_log('word_matcher.py:match_word', 'All strategies failed', {
                  'word': word, 'stem': stem, 'normalized_stem': normalized_stem, 'stem_no_suffix': stem_no_suffix}, 'E')
        # #endregion
    ...
    def match_word_with_strong(self, word_data: Dict) -> Dict:
        ...
        # #region agent log
        debug_log('word_matcher.py:match_word_with_strong', 'Processing word with pre-existing Strong number',
                  {'word': original_word, 'strong': strong_number}, 'SQLITE')
        # #endregion
        ...
        prefixes, stem = self.prefix_detector.identify_prefixes(
            cleaned_word, conservative=not has_strong)

        # Validate prefix detection: never allow more than 2 prefixes
        # Hebrew grammar allows at most: ו (conjunctive) + one preposition (ב/ל/כ/מ)
        # If we detected more than 2, limit to the first 2 (Hc + one preposition)
        if len(prefixes) > 2:
            # #region agent log
            debug_log('word_matcher.py:match_word_with_strong', 'Too many prefixes detected, limiting to first 2', {
                      'detected_prefixes': prefixes, 'word': original_word}, 'SQLITE')
            # #endregion
            prefixes = prefixes[:2]

        # #region agent log
        debug_log('word_matcher.py:match_word_with_strong', 'Prefix detection result', {
                  'prefixes': prefixes, 'stem': stem}, 'SQLITE')
        # #endregion
