#!/usr/bin/env python3
"""
Audit script to identify mismatches between raw Strong definitions and lexicon definitions.
This helps find homograph issues where the wrong BDB entry was selected.
"""

import argparse
import json
import sys
import re
import xml.etree.ElementTree as ET
from pathlib import Path

# Add the scripts directory to the path so we can import config
sys.path.insert(0, str(Path(__file__).parent))

from config import Config


def is_proper_noun_definition(def_text: str) -> bool:
    """Heuristic to identify proper-noun style Strong's glosses."""
    if not def_text:
        return False
    text = re.sub(r'\s+', ' ', def_text.strip().lower())
    patterns = [
        r'\ba place\b',
        r'\bplace in\b',
        r'\bplace of\b',
        r'\ba city\b',
        r'\ba town\b',
        r'\ba people\b',
        r'\ba nation\b',
        r'\ba tribe\b',
        r'\ba man\b',
        r'\ba woman\b',
        r'\ba son\b',
        r'\ba daughter\b',
        r'\ba king\b',
        r'\ba priest\b',
        r'\ba prophet\b',
        r'\ban angel\b',
        r'\ba river\b',
        r'\ba mountain\b',
        r'\ba valley\b',
        r'\ba desert\b',
    ]
    return any(re.search(pat, text) for pat in patterns)

def load_raw_strongs():
    """Load raw Strong's dictionary data."""
    config = Config()
    raw_strongs_path = config.RAW_DIR / 'strongs_hebrew_dict_en.json'

    with open(raw_strongs_path, 'r', encoding='utf-8') as f:
        return json.load(f)

def load_lexicon_words():
    """Load lexicon words data."""
    config = Config()
    lexicon_words_path = config.LEXICON_DIR / 'words.json'

    with open(lexicon_words_path, 'r', encoding='utf-8') as f:
        return json.load(f)


def load_lexical_index():
    """Load LexicalIndex XML and create Strong to BDB mapping."""
    config = Config()
    if not config.LEXICAL_INDEX.exists():
        return {}

    tree = ET.parse(config.LEXICAL_INDEX)
    root = tree.getroot()
    li_ns = {'li': 'http://openscriptures.github.com/morphhb/namespace'}
    mapping = {}
    for entry in root.findall('.//li:entry', li_ns):
        xref = entry.find('li:xref', li_ns)
        if xref is not None:
            strong = xref.get('strong')
            bdb = xref.get('bdb')
            if strong and bdb:
                mapping[f"H{strong}"] = bdb
    return mapping


def load_bdb_def_counts():
    """Load BDB XML and count definitions per entry id."""
    config = Config()
    if not config.BDB_XML.exists():
        return {}

    tree = ET.parse(config.BDB_XML)
    root = tree.getroot()
    ns = {'bdb': 'http://openscriptures.github.com/morphhb/namespace'}
    counts = {}
    for entry in root.findall('.//bdb:entry', ns):
        entry_id = entry.get('id')
        if not entry_id:
            continue
        defs = [d for d in entry.findall('.//bdb:def', ns) if d.text and d.text.strip()]
        counts[entry_id] = len(defs)
    return counts

def normalize_text(text):
    """Normalize text for comparison by removing extra whitespace and punctuation."""
    if not text:
        return ""
    # Remove extra whitespace, convert to lowercase
    return ' '.join(text.lower().split())

def tokenize(text: str) -> set:
    if not text:
        return set()
    cleaned = re.sub(r"[^a-zA-Z0-9\s]", " ", text.lower())
    return {t for t in cleaned.split() if len(t) > 2}


def token_overlap(a: set, b: set) -> float:
    if not a or not b:
        return 0.0
    return len(a & b) / len(a | b)


def audit_definitions(
    exclude_proper_nouns: bool = False,
    only_empty_bdb: bool = False,
    min_overlap: float = 0.0,
    explain_empty_bdb: bool = False,
):
    """Audit definitions for mismatches."""
    raw_strongs = load_raw_strongs()
    lexicon_words = load_lexicon_words()
    lexical_index = load_lexical_index() if explain_empty_bdb else {}
    bdb_def_counts = load_bdb_def_counts() if explain_empty_bdb else {}

    mismatches = []
    total_checked = 0
    skipped_proper = 0

    for strong_num, lexicon_entry in lexicon_words.items():
        if strong_num not in raw_strongs:
            continue

        raw_entry = raw_strongs[strong_num]

        # Get raw definitions
        raw_defs = []
        if 'strongs_def' in raw_entry:
            raw_defs.append(raw_entry['strongs_def'])
        if 'kjv_def' in raw_entry:
            raw_defs.append(raw_entry['kjv_def'])

        if exclude_proper_nouns:
            combined_raw = ' '.join(raw_defs)
            if is_proper_noun_definition(combined_raw):
                skipped_proper += 1
                continue

        total_checked += 1

        # Get lexicon BDB definitions
        lexicon_bdb_defs = []
        if 'definitions' in lexicon_entry:
            for def_item in lexicon_entry['definitions']:
                if def_item.get('source') == 'bdb' and 'text_en' in def_item:
                    lexicon_bdb_defs.append(def_item['text_en'])

        if only_empty_bdb and len(lexicon_bdb_defs) > 0:
            continue

        # Check if any raw definition matches any lexicon BDB definition
        raw_normalized = [normalize_text(d) for d in raw_defs]
        lexicon_normalized = [normalize_text(d) for d in lexicon_bdb_defs]

        has_match = False
        for raw_def in raw_normalized:
            for lex_def in lexicon_normalized:
                # Check for substring match (lexicon might be more detailed)
                if raw_def in lex_def or lex_def in raw_def:
                    has_match = True
                    break
            if has_match:
                break

        if not has_match:
            if min_overlap > 0:
                raw_tokens = tokenize(' '.join(raw_defs))
                bdb_tokens = tokenize(' '.join(lexicon_bdb_defs))
                if token_overlap(raw_tokens, bdb_tokens) >= min_overlap:
                    continue
            record = {
                'strong_number': strong_num,
                'lemma': lexicon_entry.get('lemma', ''),
                'raw_strongs_def': raw_entry.get('strongs_def', ''),
                'raw_kjv_def': raw_entry.get('kjv_def', ''),
                'lexicon_bdb_defs': lexicon_bdb_defs[:3],  # First 3 for brevity
                'lexicon_bdb_count': len(lexicon_bdb_defs)
            }

            if explain_empty_bdb and len(lexicon_bdb_defs) == 0:
                bdb_id = lexical_index.get(strong_num)
                if not bdb_id:
                    record['empty_bdb_reason'] = 'no_lexical_index_mapping'
                elif bdb_id not in bdb_def_counts:
                    record['empty_bdb_reason'] = 'bdb_entry_missing'
                elif bdb_def_counts.get(bdb_id, 0) == 0:
                    record['empty_bdb_reason'] = 'bdb_entry_has_no_defs'
                else:
                    record['empty_bdb_reason'] = 'unknown'
                record['bdb_id'] = bdb_id

            mismatches.append(record)

    print(f"Total entries checked: {total_checked}")
    print(f"Mismatches found: {len(mismatches)}")

    if mismatches:
        print("\nMismatches:")
        for mismatch in mismatches[:10]:  # Show first 10
            print(f"\n{mismatch['strong_number']}: {mismatch['lemma']}")
            print(f"  Raw Strong: {mismatch['raw_strongs_def']}")
            print(f"  Raw KJV: {mismatch['raw_kjv_def']}")
            print(f"  Lexicon BDB ({mismatch['lexicon_bdb_count']} defs): {', '.join(mismatch['lexicon_bdb_defs'])}")
            if mismatch.get('empty_bdb_reason'):
                print(f"  Empty BDB reason: {mismatch['empty_bdb_reason']} ({mismatch.get('bdb_id')})")

        # Save full results to file
        output_path = Path(__file__).parent / 'lexicon_audit_results.json'
        with open(output_path, 'w', encoding='utf-8') as f:
            json.dump(mismatches, f, indent=2, ensure_ascii=False)
        print(f"\nFull results saved to: {output_path}")

    if exclude_proper_nouns:
        print(f"\nSkipped proper-noun glosses: {skipped_proper}")

    return mismatches

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Audit lexicon definitions vs Strong\'s glosses')
    parser.add_argument('--exclude-proper-nouns', action='store_true', help='Skip proper-noun style Strong\'s glosses')
    parser.add_argument('--only-empty-bdb', action='store_true', help='Only report entries with zero BDB definitions')
    parser.add_argument('--min-overlap', type=float, default=0.0, help='Minimum token overlap ratio to suppress a mismatch (0-1)')
    parser.add_argument('--explain-empty-bdb', action='store_true', help='Explain why BDB definitions are empty (LexicalIndex/BDB lookup)')
    args = parser.parse_args()

    audit_definitions(
        exclude_proper_nouns=args.exclude_proper_nouns,
        only_empty_bdb=args.only_empty_bdb,
        min_overlap=args.min_overlap,
        explain_empty_bdb=args.explain_empty_bdb,
    )