...
from .batch_translator import BatchRequest
...
class LexiconProcessor:
    ...
    def process_file(
        ...
        for entry_key, entry in entries_to_process.items():
            try:
                processed, translated = self._process_entry(entry, entry_key)
                ...
                if processed > 0:
                    logger.info(
                        f"Entry {entry_key}: {translated}/{processed} definitions translated"
                    )
